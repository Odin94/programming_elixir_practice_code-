defmodule Roman do
  @doc """
  Convert the number to a roman number.
  """
  @spec numerals(pos_integer) :: String.t()
  def numerals(0), do: ""
  def numerals(number) when number >= 1000, do: "M#{numerals(number - 1000)}"
  def numerals(number) when number >= 900, do: "CM#{numerals(number - 900)}"

  def numerals(number) when number >= 500 do
    hundred_digit = number |> Integer.digits() |> List.first()
    "D" <> String.duplicate("C", hundred_digit - 5) <> numerals(number - hundred_digit * 100)
  end

  def numerals(number) when number >= 400, do: "CD#{numerals(number - 400)}"

  def numerals(number) when number >= 100 do
    hundred_digit = number |> Integer.digits() |> List.first()
    String.duplicate("C", hundred_digit) <> numerals(number - hundred_digit * 100)
  end

  def numerals(number) when number >= 90, do: "XC#{numerals(number - 90)}"

  def numerals(number) when number >= 50 do
    ten_digit = number |> Integer.digits() |> List.first()
    "L" <> String.duplicate("X", ten_digit - 5) <> numerals(number - ten_digit * 10)
  end

  def numerals(number) when number >= 40, do: "XL#{numerals(number - 40)}"

  def numerals(number) when number >= 10 do
    ten_digit = number |> Integer.digits() |> List.first()
    String.duplicate("X", ten_digit) <> numerals(number - ten_digit * 10)
  end

  def numerals(number) when number >= 9, do: "IX#{numerals(number - 9)}"
  def numerals(number) when number >= 5, do: "V#{String.duplicate("I", number - 5)}"

  def numerals(4), do: "IV#{}"

  def numerals(number), do: String.duplicate("I", number)
end
